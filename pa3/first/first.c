...
void printCacheInfo(){
	...
	printf("Number of Of: %d\n", offset_bits);
}

// DYNAMIC MEMORY

// allocate a 2D array where rows == sets and columns == blocks of a cache
size_t ** allocateCache(int numSets, int numBlocks){
  	size_t** cache =(size_t**) malloc(sizeof(size_t*) * numSets);
  	int set, block;
	for(set = 0; set < numSets; set++){
		// dynamically allocate columns (blocks)
    		cache[set] = (size_t*) malloc(sizeof(size_t) * numBlocks);
...
// free dynamic memory
void freeCache(size_t ** cache, int numSets, int numBlocks){
  	// freeing cache
  	int set, block;
	for(set = 0; set < numSets; set++){
		// set values to NULL for next use
		for (block = 0; block < numBlocks; block++){
			cache[set][block] = (size_t) NULL;
		}
		// free the block(s) at the given set
		free(cache[set]);
		cache[set] = NULL;
  	}
...
int searchCache(size_t** cache, size_t address, int numBlockOffBits, int numSetBits, int numBlocks){ 
...
size_t** updateLRU(size_t** cache, size_t address, int numBlockOffBits, int numSetBits, int numBlocks){
  	// use bit manipulation to obtain the the corresponding set and tag
	size_t setIndex = (address>>numBlockOffBits)&((1<<numSetBits)-1);
...
size_t** addToCache(size_t** cache, size_t address, int numBlockOffBits, int numSetBits, int numBlocks){ 
  	// use bit manipulation to obtain the the corresponding set and tag
	size_t setIndex = (address>>numBlockOffBits)&((1<<numSetBits)-1);
	int isAdded = 0, block;
...
void updateCache(FILE * trace_file, size_t** cache, int numBlockOffBits, int numSetBits, int numBlocks, char* replacement_policy){	
	...
	// reads until end of file
  	while((fscanf(trace_file,"%c %zx\n", &operation, &address) != EOF) && (operation != '#')){
		...
		if(searchCache(cache, address, numBlockOffBits, numSetBits, numBlocks) == 1){
			...
			// if the replacement policy is LRU, then run LRU function
				...
				// update which block has been most recently used
        			cache = updateLRU(cache, address, numBlockOffBits, numSetBits, numBlocks);
    			}
    		}
		// else is a miss
		else {
      			...
      			memory_reads++;
			// insert
      			cache = addToCache(cache, address, numBlockOffBits, numSetBits, numBlocks);
    		}
	}
}
...
int main(int argc, char *argv[]){
	...
	// dynamically alloce the cache
	size_t** cache = allocateCache(number_of_sets, number_of_blocks);

	// read from file 
  	updateCache(infile, cache, offset_bits, set_bits, number_of_blocks, replacementPolicy);

	// print out output specified by project description	
	printSimulationCounts();

  	// freeing cache
	freeCache(cache, number_of_sets, number_of_blocks);
